...
class CrawlParagraph(HTMLParser):
    ...
    def __init__(self, get_url, url, file_index=-1):
        """ (self, bool, str)
                ...
                for attr in attrs:
                    if attr[0] == 'href':
                        ...
                        if url[0:7] == 'http://' and url[-4:] != '.pdf' \
                            ...
                            if not url in CrawlParagraph.url_list:
                                print '[INFO] href = ', url
                                with open(store_url, 'a') as writing_file:
                                    writing_file.write(url + '\n')
                                CrawlParagraph.url_list.append(url + '/')
                            else:
    ...
    def handle_data(self, data):
        if DEBUG or CrawlParagraph.filter(data):
            if not self.get_url:
                # @formatter:off
                if self.found_p:
                # if self.found_a and (
...
def main():
    ...
    if DEBUG:
        ...
        url_parser = CrawlParagraph(True, url)
        ...
        with open(store_url, 'r') as reading_file:
            for pageUrl in reading_file:
                ...
                parser = CrawlParagraph(False, url, i)
                # The "feed" method's parameter had better be Unicode
                # ,so use decode('utf-8') to restore to Unicode string.
                try:
