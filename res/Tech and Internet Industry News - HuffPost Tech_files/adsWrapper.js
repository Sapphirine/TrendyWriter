...
adUACInit()
function atwIsInList(m){
 var li=new Array("93318430","93414165"),l=li.length;
 for (var i=0;i<l;i++){
   if(m.indexOf(li[i])>-1)return i+1;
 }
 return false;
}
...
if (atwIsInList(f.mn)){
var e=new Date().getTime(),e2=e-f.timing,i=document.createElement('script'),pr=location.protocol+'//';
if (pr.indexOf('http')<0)pr='http://';
if (adsDev==1){
 i.src=pr+'mads.at.atwola.com/adcall?mpid=348-d-d-e;rettype=js;callProtocol=3.0;networkId='+adsNt+';placementid='+adsPl+';alias=mail_time_test;random='+adsScr+';sizeId=-1;size=1x1;noperf=1;cfp=1;noaddonpl=y;kvmn='+f.mn+';kvismob='+adsDev+';kvmailtt='+e2+';extmirroring=0;target=_blank;grp='+adsScr;
}
else{
 i.src=pr+'at.atwola.com/addyn/3.0/5113.1/221794/0/-1/size=1x1;noperf=1;alias=mail_time_test;cfp=1;noaddonpl=y;kvismob='+adsDev+';kvmn='+f.mn+';kvmailtt='+e2+';grp='+adsScr;
}
